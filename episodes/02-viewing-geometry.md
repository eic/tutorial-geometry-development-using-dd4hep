...
Note: The version of JSROOT on the [ANL geometry viewer](https://eic.phy.anl.gov/geoviewer/) is older than the [ROOT JSROOT viewer](https://root.cern/js/latest) version so some differences in the visualisation may be present.
...
- Open the exported ROOT file in the geometry viewer at the [ANL geometry viewer](https://eic.phy.anl.gov/geoviewer/) or the [ROOT JSROOT viewer](https://root.cern/js/latest).

::::::::::::::: solution

For example `dd_web_display -o vertex_geometry.root --export $DETECTOR_PATH/epic_vertex_only.xml`
produces `vertex_geometry.root`; loading that file in either viewer shows only the vertex barrel,
which renders faster and is easier to inspect than the full `epic.xml`.
